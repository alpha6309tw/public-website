...
此檔案是網站選單的「藍圖」，決定了導覽列的文字、連結與層級。

### 1. 欄位意義
*   **`name:`** 顯示在網頁上的選單文字。
*   **`link:`** 點擊後跳轉的網頁路徑（網站內網頁以 `/` 開頭，如 `/pages/about`；外部網站填完整網址）。
*   **`dropdown:`** 若該項目包含子選單，則使用此欄位。

### 2. 縮排規則（重要！）
此檔案對空格非常敏感，**請務必遵守以下規則**：
*   **層級關係：** 子選單項目必須比上一層**多縮排 2 個空格**。
*   **禁止使用 Tab：** 縮排只能使用半形空格。
*   **對齊一致：** 同一層級的項目（例如所有第一層選單），左側必須完全對齊。
...
- name: 讀經介紹
  link: "#"
  dropdown:
    - name: 關於讀經  # 這裡縮排 2 個空格
      link: /pages/introduction
    - name: 如何帶領
      link: /pages/how-to-guide
```
...
## 如何修改現有頁面 (使用 GitHub 網頁介面)

如果您想要修改網站上的固定頁面（例如：關於我們、出版品、讀經書籤等），請依照以下步驟操作：

### 1. 找到對應的檔案
網站的網址與 GitHub 上的檔案路徑有直接的對應關係。
*   **網址範例**：`https://www.alpha.org.tw/pages/出版品`
*   **檔案路徑**：`pages/出版品.md`
*   **對應規則**：網址中 `/pages/` 後面的名稱，就是 `pages` 資料夾內對應的 `.md` 檔案名稱。

### 2. 修改步驟
1.  **進入目錄**：在 GitHub 專案首頁，點擊進入 `pages` 資料夾。
2.  **選擇檔案**：找到您要修改的檔案（例如 `出版品.md`）並點擊進入。
3.  **開啟編輯**：點擊檔案內容右上角的**鉛筆圖示 (Edit this file)**。
4.  **編輯內容**：在編輯區直接修改 Markdown 文字或連結。
    *   請注意不要更動最上方被 `---` 包圍的屬性區域（Front Matter），除非您需要修改標題或版型。
5.  **預覽與提交**：
    *   點擊「預覽 (Preview)」確認修改結果。
    *   點擊右上方綠色的「提交變更... (Commit changes...)」按鈕。
    *   輸入簡短的修改說明（例如：更新出版品清單）。
    *   點擊「提交變更 (Commit changes)」完成。
6.  **等待發布**：同樣需要等待約 3 到 5 分鐘，變更才會反映在網站上。

## 測試與預覽頁面 (實驗區)
如果您有不確定的內容或排版（例如：複雜的表格、多張圖片排列），可以先在「測試頁」進行實驗，確認沒問題後再移至正式頁面。

*   **測試檔案路徑**：`pages/test.md`
*   **實際預覽網址**：[https://www.alpha.org.tw/pages/test](https://www.alpha.org.tw/pages/test)

**操作建議：**
1. 先依照修改步驟編輯 `pages/test.md` 並提交變更。
2. 等待部署完成後，到上述網址查看網頁效果。
3. 效果理想後，再將內容複製、貼上到正確的目標檔案（例如 `pages/出版品.md`）。
